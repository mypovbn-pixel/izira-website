...
function bruneiMonthBounds(){
  const parts=new Intl.DateTimeFormat('en-CA',{timeZone:'Asia/Brunei',year:'numeric',month:'2-digit'}).formatToParts(new Date())
  const year=Number(parts.find(p=>p.type==='year')?.value)
  const month=Number(parts.find(p=>p.type==='month')?.value)
  const nextMonth=month===12?1:month+1
  const nextYear=month===12?year+1:year
  const start=new Date(`${year}-${String(month).padStart(2,'0')}-01T00:00:00+08:00`).toISOString()
  const end=new Date(`${nextYear}-${String(nextMonth).padStart(2,'0')}-01T00:00:00+08:00`).toISOString()
  return {start,end}
}
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    const{data:business,error:businessError}=await supabase.from('businesses')
      .select('id,is_active,plan,pickup_enabled,delivery_enabled,bank_name,account_name,account_number,pickup_address')
      .eq('slug',String(slug).toLowerCase()).eq('is_active',true).single()
    if(businessError||!business)return json({error:'Store not found'},404)

    const limits:Record<string,number|null>={free:30,starter:150,pro:null,business:null}
    const limit=limits[business.plan]??30
    if(limit!==null){
      const {start,end}=bruneiMonthBounds()
      const {count,error:countError}=await supabase.from('orders').select('id',{count:'exact',head:true})
        .eq('business_id',business.id).gte('created_at',start).lt('created_at',end).neq('order_status','cancelled')
      if(countError)throw countError
      if((count||0)>=limit)return json({error:'This store has reached its monthly order limit. Please contact the seller directly.',code:'PLAN_ORDER_LIMIT',plan:business.plan,limit},403)
    }
