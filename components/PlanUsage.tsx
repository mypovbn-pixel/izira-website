...
export default function PlanUsage(){
 ...
 if(loading)return <main className="shell"><div className="card" style={{marginTop:60}}>Checking your KADAI plan…</div></main>;
 if(!business||!meta)return null;

 return <main className="shell" style={{paddingTop:34,paddingBottom:70}}>
  <div style={{display:"flex",justifyContent:"space-between",gap:20,alignItems:"center",flexWrap:"wrap"}}>
   <div><div className="eyebrow">{business.name}</div><h1 style={{fontSize:46,margin:"6px 0 10px"}}>Money & plan</h1><p className="muted">Your order limit resets each Brunei calendar month.</p></div>
   <Link href="/dashboard" className="btn secondary">← Back to KADAI</Link>
  </div>
