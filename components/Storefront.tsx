...
export default function Storefront({slug,products,slots,pickupEnabled,deliveryEnabled,pickupAddress}:Props){
 ...
 if(submitted){const order=submitted.order;const payment=submitted.payment||{};const statusHref=order.public_token?`/${slug}/order/${order.public_token}`:null;return <div className="card" style={{maxWidth:620,margin:"60px auto",textAlign:"center"}}><div style={{fontSize:52}}>✓</div><h1>Order received</h1><p>Your order number is <b>#{order.order_number}</b>.</p><p className="muted">Total: BND {Number(order.total).toFixed(2)} · {receiptUploaded?"Receipt uploaded":"Awaiting payment"}.</p>{payment.bank_name&&payment.account_number?<div className="card" style={{margin:"22px 0",textAlign:"left"}}><div className="eyebrow">BANK TRANSFER</div><p><b>{payment.bank_name}</b><br/>{payment.account_name}<br/><span style={{fontSize:22,fontWeight:800}}>{payment.account_number}</span></p><p className="muted">Transfer the exact amount, then upload your receipt below.</p></div>:<p className="muted">The seller will send payment instructions directly.</p>}{!receiptUploaded&&<div className="card" style={{margin:"18px 0",textAlign:"left"}}><b>Upload payment receipt</b><p className="muted">JPG, PNG, WebP or PDF · maximum 5 MB.</p><input type="file" accept="image/jpeg,image/png,image/webp,application/pdf" onChange={e=>setReceipt(e.target.files?.[0]||null)}/>{receipt&&<button className="btn" style={{marginTop:14}} disabled={receiptLoading} onClick={uploadReceipt}>{receiptLoading?"Uploading…":"Submit receipt"}</button>}</div>}{receiptUploaded&&<div className="card" style={{margin:"18px 0",background:"#eef7ee"}}><b>Receipt submitted.</b><p className="muted">The seller can now verify your payment from their dashboard.</p></div>}{fulfilment==="pickup"&&submitted.pickup_address&&<div className="card" style={{textAlign:"left"}}><b>Pickup</b><p className="muted">{submitted.pickup_address}</p></div>}{statusHref&&<div className="card" style={{margin:"18px 0"}}><b>Save your private order link</b><p className="muted">Use it anytime to check payment and preparation status.</p><Link className="btn" href={statusHref}>View order status</Link></div>}{error&&<p style={{color:"crimson"}}>{error}</p>}<button className="btn secondary" style={{marginTop:18}} onClick={()=>{setSubmitted(null);setCart({});setSlotId("");setReceipt(null);setReceiptUploaded(false)}}>Back to store</button></div>}
