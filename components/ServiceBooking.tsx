...
export default function ServiceBooking({slug,businessName,businessWhatsApp,services}:Props){
  ...
  function chooseService(id:string){
    ...
    setPickup("");setDestination("");setPassengers("1");setItemDescription("");setTripDirection("one_way");setReturnAt("");setRecurrenceType("one_off");setRecurringUntil("");setRecurrenceWeekdays([]);
  }
  ...
  return <section className={styles.layout}>
    ...
    <div className={`card ${styles.form}`}>
      ...
      {isRouteService&&<>
        ...
        {isTransport&&<div className="field"><label>Passengers</label><input type="number" min="1" max={selected?.max_passengers||undefined} value={passengers} onChange={e=>setPassengers(e.target.value)}/></div>}
      ...
      <div className="field"><label>Notes</label><textarea rows={3} value={note} onChange={e=>setNote(e.target.value)} placeholder={isRouteService?"Child name, gate instructions, luggage, special timing…":"Anything the seller should know?"}/></div>
